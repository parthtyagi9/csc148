...
class CallHistory:
    ...
    def register_outgoing_call(self, call: Call) -> None:
        ...
        """
        tup = call.get_bill_date()
        if tup not in self.outgoing_calls:
            self.outgoing_calls[tup] = [call]
        else:
            self.outgoing_calls[tup].append(call)

    def register_incoming_call(self, call: Call) -> None:
        """ Register a Call <call> into this incoming call history
        """
        tup = call.get_bill_date()
        if tup not in self.incoming_calls:
            self.incoming_calls[tup] = [call]
        else:
            self.incoming_calls[tup].append(call)
