...
def process_event_history(log: dict[str, list[dict]],
    ...
    for event_data in log['events']:
        if event_data['type'] == 'call':
            ...
            call_object = Call(src_number, dst_number, time, duration, src_loc,
                               dst_loc)
            if (current_time.month != time.month
                    or current_time.year != time.year):
                new_month(customer_list, time.month, time.year)
            caller = find_customer_by_number(src_number, customer_list)
            caller.make_call(call_object)
            receiver = find_customer_by_number(dst_number, customer_list)
            receiver.receive_call(call_object)
