...
def test_data_size_python_files_folder():
    ...
    # Sum of the sizes of the python_files folder and py files subfolder
    expected_size = 27 + 30 + 216 + 189  # File sizes in bytes
...
def test_change_size_leaf_increase():
    ...
    leaf = TMTree('leaf', [], 10)
    leaf.change_size(0.1)  # Increase by 10%
    assert leaf.data_size == 11, "Leaf size should increase by 10%."
...
def test_delete_file_updates_sizes_and_rectangles():
    ...
    folder_with_file = TMTree('FolderWithFile', [file])
    empty_folder = TMTree('EmptyFolder', [])
    root = TMTree('Root', [folder_with_file, empty_folder])
    ...
    assert file not in folder_with_file._subtrees
...
def test_rectangle_update_on_collapsed_subtrees():
    ...
    assert not parent._expanded
    parent.update_rectangles((0, 0, 100, 100))
    assert parent.rect == (0, 0, 100, 100), "Parent's rectangle should be updated."
    assert child.rect == (0, 0, 100, 100), "Collapsed child's rectangle should be minimized."
...
def test_collapse_parent_of_selected_leaf():
    child = TMTree('Child', [], 10)
    parent = TMTree('Parent', [child], 10) # Parent is initially expanded
    parent.expand()
